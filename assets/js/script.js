...
// here use my projects file which use json format -- let's start 

// (async () => {
//     try {
//         // Fetch the JSON data from the local file
//         const link = `https://proxy.techzbots1.workers.dev/?u=https://api.prodyun.me/recent`; // Correct path to your local JSON file
...
//         const projects = await response.json(); // Parsing the JSON data

//         // Iterate over each project and create HTML structure
...
//                             </a>
//                             <a href="${project.external_link}" target="_blank" class="external-link">
...
//             // Append the dynamically created card HTML to the container
...
(async () => {
    try {
        // Fetch the JSON data from the local file
        const link = `assets/Json/project.json`; // Correct path to your local JSON file
        const response = await fetch(link);
        ...
        projects.forEach(project => {
            ...
            // Append the dynamically created card HTML to the container
            document.getElementById('load-more').insertAdjacentHTML('beforeend', cardHTML);
        });
    } catch (error) {
        console.error('Error fetching the JSON file:', error);
    }
